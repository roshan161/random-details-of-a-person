...
for num in range(100):
    ...
    street_nums = random.randint(100, 999)
    street = random.choice(street_names)
    city = random.choice(fake_cities)
    state = random.choice(states)
    zip_code = random.randint(10000, 99999)
    address = f'{street_nums} {street} St., {city} {state} {zip_code}'

    email = first.lower() + last.lower() + '@usegoogle.com'

    print(f'His name is {first} {last}.\nHis phone number is {phone}.\nHis address is {address}.'
          f'\nHe uses the email named {email}.\n')
